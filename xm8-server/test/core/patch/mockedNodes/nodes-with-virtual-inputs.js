...
let sum1 = sum();
param(sum1, '0', paramTypesById.VIRTUALINPUT, pureVirtual1.id);
param(sum1, '1', paramTypesById.VIRTUALINPUT, virtualWithPhysical.id);

let sum2 = sum();
param(sum2, '0', paramTypesById.VIRTUALINPUT, pureVirtual1.id); // already added by sum1
param(sum2, '1', paramTypesById.VIRTUALINPUT, pureVirtual2.id);
link(sum1, sum2, '2');

let unreachableInvert = invert();
param(unreachableInvert, '0', paramTypesById.VIRTUALINPUT, pureVirtual3.id); // pure virtual but unreachable node
